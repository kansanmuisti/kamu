...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        print "Populating PlenarySession term fields..."
        for pl in orm.PlenarySession.objects.all():
            query = Q(begin__lte=pl.date)
            query &= Q(end__isnull=True) | Q(end__gte=pl.date)
            pl.term = orm.Term.objects.get(query)
            pl.save()
